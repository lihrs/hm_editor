				...
				CKEDITOR.tools.setTimeout( function() {
					var panelLoad = CKEDITOR.tools.bind( function() {
						...
						if ( block.autoSize ) {
							...
							var elements = ((CKEDITOR.env.webkit || CKEDITOR.env.edge) ? block.element : panelDoc.getBody()).$;

							var shortcutItems = $(elements).find('.hm-shortcut-label,.hm-shortcut-display');
							// 先去掉 菜单栏中的二级菜单中的快捷键 的浮动样式, 以计算菜单宽度
							shortcutItems.removeClass('hm-shortcut-display').addClass('hm-shortcut-label');
							// 获取 菜单栏中的二级菜单 的宽度
							var width = elements.scrollWidth;
							// 恢复 菜单栏中的二级菜单中的快捷键 的浮动样式, 以在右侧显示
							shortcutItems.removeClass('hm-shortcut-label').addClass('hm-shortcut-display');

							// 二级菜单的 title
							for (var _ = 0; _ < shortcutItems.length; _++) {
								var shortcutParent = shortcutItems[_].parentNode;
								var shortcut = shortcutItems[_].innerHTML;
								shortcut = shortcut ? ' (' + shortcut + ')' : '';
								if (shortcutParent.firstChild && shortcutParent.firstChild.nodeType === CKEDITOR.NODE_TEXT) {
									shortcutParent.title = shortcutParent.firstChild.data + shortcut;
								}
							}
