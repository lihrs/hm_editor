		...
		proto: {
			setData: function( data, isSnapshot ) {
				...
				if ( isSnapshot ) {
					...
					if ( !fullPage ){
                        var editorConfig = editor.HMConfig||{};
                        headExtra += CKEDITOR.tools.buildStyleHtml( editor.config.contentsCss,editorConfig.sdkHost );
                    }
					...
					if ( fullPage ) {
						...
						data = config.docType +
							...
							'<head>' +
								...
								baseTag +
								headExtra +
								'<style type="text/css">._emr_diagonal:before{content: "";position: absolute;width: 1px;height:0px;top:0;left:0;background-color: black;display: block;transform-origin: top;-webkit-transform-origin: top;-webkit-backface-visibility: hidden;}</style>' +
							'</head>' +
