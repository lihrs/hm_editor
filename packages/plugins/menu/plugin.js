	...
	CKEDITOR.menu = CKEDITOR.tools.createClass( {
		...
		_: {
			...
			onClick: function( item ) {
				...
				if ( item.onClick )
					...
					// 粘贴前不执行 focus, 否则如果在聚合病历的表格中的换行符后面执行 focus 会变选区.
					...
					switch (item.command) {
						...
						case '牙位':
						case 'copy':
							additionalData = {doNotFocus: true};
							break;
					}
					this.editor.execCommand(item.command, additionalData);
				}
			},
		...
		proto: {
			...
			show: function( offsetParent, corner, offsetX, offsetY ) {
				...
				// Show the panel.
				if ( this.parent ){
					var a=this.parent._.panel.element.$.offsetLeft;
					var b=this.parent._.panel.element.$.clientWidth;
					var c=$("#cke_editorSpace").width();
					if (c-a-b<130) {//右键菜单的二级菜单如果在右边放不下则改放左边，防止和一级菜单重叠
						offsetX = -260;
					}
					if (this.parent.parent) {
						a=this.parent.parent._.panel.element.$.offsetLeft;
						b=this.parent.parent._.panel.element.$.clientWidth;
						if (c-a-b<130) {//右键菜单的三级菜单如果在右边放不下则改放左边，防止和二级菜单重叠
							offsetX = -260;
						}
					}
